...
class UsaWestpointParser(BaseParser):
    name = 'westpoint'
    
    # 站点id
    site_id = "b85d5cb9-7fe5-48c0-8eca-c9e59eec469e"
    # 站点名
    site_name = "西点军校"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "b85d5cb9-7fe5-48c0-8eca-c9e59eec469e", "source_name": "西点军校", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("86dcff31-b945-45c8-8458-96821de73472", "新闻", "https://www.westpoint.edu/news/all-cadet-news", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="views-row"]/div/section/div/div[2]/a/@href').extract() or []
    ...
    def get_author(self, response) -> list:
        authors = response.xpath('//span[@class="text-muted"]/text()').extract_first()
        authors = authors.split("-")[0].strip().strip("By ")
        return [authors]

    def get_pub_time(self, response) -> str:
        try:
            _time = response.xpath('//span[@class="text-muted"]/text()').extract_first()
    ...
    def get_content_media(self, response) -> list:
        content = []
        news_tags = response.xpath('//div[@class="carousel-inner"]//img'
                                   '|//div[@class="post-body"]//p') or []
