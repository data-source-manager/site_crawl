...
class VnvNexPressParser(BaseParser):
    name = 'vn_vnexpress'
    
    # 站点id
    site_id = "a934a240-48a4-43c9-b3b4-5750ab8938c8"
    # 站点名
    site_name = "越南每日快讯"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "a934a240-48a4-43c9-b3b4-5750ab8938c8", "source_name": "越南每日快讯", "direction": "pa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8ef980-fe6f-11ec-a30b-d4619d029786", "政治", "https://vnexpress.net/thoi-su/chinh-tri-p1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h2[@class="title-news"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="title-detail"]/text()').extract_first(default="")
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//h4[@class="item-tag"]/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="fig-picture"]/picture/img|//div[@class="sidebar-1"]/p|'
                                   '//div[@class="sidebar-1"]/article/div/*')
        if news_tags:
