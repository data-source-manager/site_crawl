...
class SG_sgpcgovParser(BaseParser):
    name = 'sg_sgpcgov'
    
    # 站点id
    site_id = "717015f7-6b0c-4ae6-b882-2c8be5c7f57a"
    # 站点名
    site_name = "新加坡政事中心"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "717015f7-6b0c-4ae6-b882-2c8be5c7f57a", "source_name": "新加坡政事中心", "direction": "sg", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("91237068-2f72-11ed-a768-d4619d029786", "新闻中心", "https://www.sgpc.gov.sg/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath("//div[@class='wrap']/div[@class='press-title']/p/a/@href").extract() or []
        if news_urls:
    ...
    def get_title(self, response) -> str:
        title = response.xpath("//div[@class='wrap']/h2/text()").extract_first(default="")
        return title.strip() if title else ""

    def get_author(self, response) -> list:
        author_list = []
        author_str = response.xpath("//span[@class='name']/text()").get()
        if author_str:
    ...
    def get_pub_time(self, response) -> str:
        ...
        """
        datePublished_str = response.xpath("//span[@class='press-info-date']/text()").get()
    ...
    def get_content_media(self, response) -> list:
        content = []
        news_tags = response.xpath("//div[@class='wrap']/*|//div[contains(@id,'ftn')]/*|//div[@class='left']//a")
        if news_tags:
            for news_tag in news_tags:
                if news_tag.root.tag in ["h3", "h5", "span", "p", "blockquote"] and news_tag.root.get("class",
                    ...
                    if news_tag.xpath("./self::a[contains(@href,'.pdf')]/@href"):
