...
class UyCuriParser(BaseParser):
    name = 'curi'
    
    # 站点id
    site_id = "bd1b21a4-e2e3-4aac-9246-0a8c0f4cce0e"
    # 站点名
    site_name = "乌拉圭国际关系委员会"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "bd1b21a4-e2e3-4aac-9246-0a8c0f4cce0e", "source_name": "乌拉圭国际关系委员会", "direction": "uy", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f57e0-fe6f-11ec-a30b-d4619d029786", "消息", "http://curi.org.uy/noticias-internacionales/page/1/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="col col_12_of_12"]//h4/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="entry_title entry-title"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//span[@class="meta_date date updated"]/a/text()').extract_first()
        if time_:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//div[@class="entry_tags categories"]/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="entry_media"]/a/img|//div[@class="entry_content"]/*')
        if news_tags:
