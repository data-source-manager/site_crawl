...
class TwLygovParser(BaseParser):
    name = 'lygov'
    
    # 站点id
    site_id = "f097e257-ab49-49dd-9e71-f5698a22f780"
    # 站点名
    site_name = "台湾立法院"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "f097e257-ab49-49dd-9e71-f5698a22f780", "source_name": "台湾立法院", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("e700ede0-7607-11ed-ad4d-d4619d029786", "新闻讯息", "https://www.ly.gov.tw/Pages/TopicList.aspx?nodeid=5255&idx=0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="col-md-12 six-pointFile"]/div[@class="row"]')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('./div/a/@href').get()
                rsp_time = news_url.xpath('./div[2]/text()').get()
                url = urljoin(response.url, rsp_url)
                self.Dict[url] = {'time': rsp_time}
                yield url

    def get_title(self, response) -> str:
        title = response.xpath('//meta[@name="DC.Description"]/@content').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="col-md-12"]/p')
        if news_tags:
