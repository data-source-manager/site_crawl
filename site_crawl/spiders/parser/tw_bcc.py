...
class TwBccParser(BaseParser):
    name = 'bcc'
    
    # 站点id
    site_id = "8f91f0eb-1c8b-45c3-8c45-a2c38d06e7fd"
    # 站点名
    site_name = "中广新闻网"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "8f91f0eb-1c8b-45c3-8c45-a2c38d06e7fd", "source_name": "中广新闻网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("c3dba5f6-764f-40fa-a94c-79bda8b9a753", "首页", "https://www.bcc.com.tw/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[contains(@class,"tt")]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                url = urljoin(response.url, news_url)
                if "www.bcc.com.tw" not in url:
                    continue
                yield url
    ...
    def get_pub_time(self, response) -> str:
        ...
        """
        time_ = response.xpath('//div[@class="tt27"]/text()').extract_first().replace("報導", "").strip()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="ma7"]/text()|'
                                   '//div[@class="ma7"]/*|'
                                   '//div[@class="ma7"]/div[contains(@class,"im4")]')
        if news_tags:
