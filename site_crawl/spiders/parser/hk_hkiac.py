...
class HkHkiacParser(BaseParser):
    name = 'hkiac'
    
    # 站点id
    site_id = "9ca2f2ef-a91e-4205-9a0c-0699c21eaff9"
    # 站点名
    site_name = "香港国际仲裁中心"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "9ca2f2ef-a91e-4205-9a0c-0699c21eaff9", "source_name": "香港国际仲裁中心", "direction": "hk", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d918a-fe6f-11ec-a30b-d4619d029786", "新闻", "https://www.hkiac.org/zh-hant/news", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h2[@property="dc:title"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//meta[@property="dc:title"]/@content').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//div/span[@property="dc:date"]/@content').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="node node-news node-promoted clearfix"]/*|'
                                   '//div[@class="node node-news clearfix"]/div[8]/*|'
                                   '//div[@class="node node-news clearfix"]/*')
        if news_tags:
