...
class UsaStripeSParser(BaseParser):
    name = 'stripes'
    
    # 站点id
    site_id = "8ac5b016-8a69-4914-9249-1ffb8adf9a2f"
    # 站点名
    site_name = "星条旗报"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "8ac5b016-8a69-4914-9249-1ffb8adf9a2f", "source_name": "星条旗报", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("738467bf-981b-3702-9d20-5b1f24e785b8", "退伍军人", "", "政治"),
            ("4a8f194c-fe6f-11ec-a30b-d4619d029786", "退伍军人/新闻", "https://www.stripes.com/veterans/news/load-more-stories?offset=0&count=10&total=117", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="image-on-right"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_author(self, response) -> list:
        ...
        authors = response.xpath('//div[@class="secondary-font dn dib ttu b"]/span/text()').extract()
        if authors:
            for au in authors:
                if au.strip():
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//span[@itemprop="datePublished"]/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="article-storyline"]/p|//div[@class="inline-image align- "]/img')
        if news_tags:
