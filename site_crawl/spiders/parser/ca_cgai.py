...
class CaCgaiParser(BaseParser):
    name = 'cgai'
    
    # 站点id
    site_id = "9c355203-1c36-4ebf-84ff-36c3e54a7bd6"
    # 站点名
    site_name = "加拿大全球事务研究所"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "9c355203-1c36-4ebf-84ff-36c3e54a7bd6", "source_name": "加拿大全球事务研究所", "direction": "ca", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f399a-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://www.cgai.ca/press_releases", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//td[@valign="top"]/p/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//strong/u/text()|//meta[@name="Title"]/@content|'
                               '//span/strong/text()').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="text-content"]/*')
        if news_tags:
