...
class UsaRockefellerFoundationParser(BaseParser):
    name = 'usa_rockefellerfoundation'
    
    # 站点id
    site_id = "98fe0793-de2f-cedc-e03c-3cac71af8c91"
    # 站点名
    site_name = "洛克菲勒财团"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "98fe0793-de2f-cedc-e03c-3cac71af8c91", "source_name": "洛克菲勒财团", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("17ad2bfe-311a-44d3-95cd-6c800c77fb71", "新闻 & 演讲", "https://www.rockefellerfoundation.org/news/page/1/?post_type=news_post&filter_news_types=25", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="post-title"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@ class="has-content-area"]/@data-title').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="has-content-area"]/*')
        if news_tags:
