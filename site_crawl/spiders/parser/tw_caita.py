...
class TwCaitaParser(BaseParser):
    name = 'caita'
    
    # 站点id
    site_id = "456902c9-a079-4ea7-8bc3-92bb56ce0817"
    # 站点名
    site_name = "中华民国产业科技发展协进会"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "456902c9-a079-4ea7-8bc3-92bb56ce0817", "source_name": "中华民国产业科技发展协进会", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f60d2-fe6f-11ec-a30b-d4619d029786", "最新消息", "https://www.caita.org.tw/news.aspx", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="container"]/div[@class="row"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//div[@class="pageGrid"]/h6/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="divDetails"]/*')
        if news_tags:
