...
class UsaC3fParser(BaseParser):
    name = 'c3f'
    # 站点id
    site_id = "49230694-89d3-4c18-bf2c-2dafcc665e28"
    # 站点名
    site_name = "第三舰队"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "49230694-89d3-4c18-bf2c-2dafcc665e28", "source_name": "第三舰队", "direction": "jp", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8e2493-fe6f-11ec-a30b-d4619d029786", "新闻", "https://www.c3f.navy.mil/News/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="info"]/@data-url').extract() or ""
        if news_urls:
    ...
    def parse_text(self, news_tag) -> list:
        ...
        if cons:
            for x in cons:
                dic = {}
                if x.strip():
                    if "For more information" not in x.strip():
                        dic['data'] = x.strip()
                        dic['type'] = 'text'
                        new_cons.append(dic)
