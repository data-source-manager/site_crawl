...
class ItCmCCParser(BaseParser):
    name = 'cmcc'
    
    # 站点id
    site_id = "90b122ff-39bd-451e-884a-eb75396541c6"
    # 站点名
    site_name = "欧洲-地中海气球变化中心"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "90b122ff-39bd-451e-884a-eb75396541c6", "source_name": "欧洲-地中海气球变化中心", "direction": "it", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8cf13a-fe6f-11ec-a30b-d4619d029786", "消息", "https://www.cmcc.it/it/category/articolo/page/1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h5[@class="entry-title"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h3[@class="page-title h3"]/text()').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="entry-thumbnail"]//img|//div[@class="entry-content"]//img|'
                                   '//div[@class="entry-content"]/*|//div[@class="entry-content"]/p/a')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "a", "strong"]:
