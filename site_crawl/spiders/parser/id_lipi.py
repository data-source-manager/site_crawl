...
class IdLipiParser(BaseParser):
    name = 'lipi'
    
    # 站点id
    site_id = "33ee6d71-6912-4c7e-9195-b8c8643efba2"
    # 站点名
    site_name = "印度尼西亚科学院"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "33ee6d71-6912-4c7e-9195-b8c8643efba2", "source_name": "印度尼西亚科学院", "direction": "id", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("cb995f87-0af6-4fd6-8327-1ebc0e9deb1a", "Berita", "http://lipi.go.id/berita/index/0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="col-xs-12 col-md-7"]/h4/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//div[@class="blog-single-head-date"]/a/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="blog-single-img"]/img|//div[@style="text-align: justify;"]|'
                                   '//div[@style="text-align: justify;"]/*|//div[@style="text-align: justify;"]/a')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "div", "em"]:
