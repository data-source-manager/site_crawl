...
class EgCedEjParser(BaseParser):
    name = 'eg_cedej'
    
    # 站点id
    site_id = "4eae3faf-6df7-43ec-a4cf-eeb8a7c08c5e"
    # 站点名
    site_name = "经济与社会文献研究中心"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "4eae3faf-6df7-43ec-a4cf-eeb8a7c08c5e", "source_name": "经济与社会文献研究中心", "direction": "eg", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f2522-fe6f-11ec-a30b-d4619d029786", "新闻", "http://cedej-eg.org/index.php/category/actualites/page/1/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="blog-content wf-td"]')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('./h3[@class="entry-title"]/a/@href').get()
                rsp_time = news_url.xpath('.//time/@datetime').get()
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="entry-content"]/*|//div[@class="entry-content"]//a/img|'
        ...
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "a", "strong", "div"]:
