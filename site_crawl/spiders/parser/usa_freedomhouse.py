...
class UsaFreedomhouseParser(BaseParser):
    name = 'freedomhouse'
    
    # 站点id
    site_id = "1b8ccf18-b3dc-66f4-d76e-dfe9757ca92a"
    # 站点名
    site_name = "自由之家"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "1b8ccf18-b3dc-66f4-d76e-dfe9757ca92a", "source_name": "自由之家", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8dd1ea-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://freedomhouse.org/news-perspectives/press-releases-statements/press-releases", "政治"),
            ("4a8dd172-fe6f-11ec-a30b-d4619d029786", "观点", "https://freedomhouse.org/news-perspectives/perspectives", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="views-row"]/div/a[1]/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//span[@class="article-date"]/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="article-image grid"]//img|'
                                   '//div[@class="lovey-block text-paragraph "]/div/div[2]/*|'
                                   '//div[@class="lovey-block text-paragraph "]/div/div/*')
        if news_tags:
