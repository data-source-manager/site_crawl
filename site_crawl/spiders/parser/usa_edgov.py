...
class UsaEdGovParser(BaseParser):
    name = 'usa_edgov'
    
    # 站点id
    site_id = "0edebc5d-8c43-18b5-bd7d-43b9b7caa1ec"
    # 站点名
    site_name = "美国教育部官网"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "0edebc5d-8c43-18b5-bd7d-43b9b7caa1ec", "source_name": "美国教育部官网", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f1474-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://www.ed.gov/news/press-releases?page=0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h2[@class="field-content"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="pane-content"]/h1/text()').extract_first(default="")
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//a[@class="press-room-padding-span"]/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
