...
class RuPircenterParser(BaseParser):
    name = 'pircenter'
    
    # 站点id
    site_id = "8e3e77ad-a4af-4592-8363-4e70169c974b"
    # 站点名
    site_name = "俄罗斯政治研究中心"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "8e3e77ad-a4af-4592-8363-4e70169c974b", "source_name": "俄罗斯政治研究中心", "direction": "ru", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d1458-fe6f-11ec-a30b-d4619d029786", "新闻", "http://pircenter.org/news", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="news-item__pict"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//span[@class="breadcrumb_last"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        times_ = response.xpath('//div[@class="inner-main__date"]/text()').extract_first()
        day, mt, year = re.findall('(\d{1,2}) (.*)я (\d{4})', times_)[0]
        time_ = year + '-' + translatetTime.ruDict[mt] + '-' + day
        if time_:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//div[@class="universal-tags"]/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="content-text"]/*|//div[@class="universal-body"]/*|'
                                   '//div[@class="universal-body"]/p/a')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "strong", "em", "a"]:
