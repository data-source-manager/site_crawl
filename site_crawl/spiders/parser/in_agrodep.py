...
class InAgrodepParser(BaseParser):
    name = 'agrodep'

    # 站点id
    site_id = "ddffe269-a794-4612-b69d-b9177b15b405"
    # 站点名
    site_name = "非洲增长与发展政策模型联盟"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "ddffe269-a794-4612-b69d-b9177b15b405", "source_name": "非洲增长与发展政策模型联盟", "direction": "in", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块主题)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f3fe4-fe6f-11ec-a30b-d4619d029786", "新闻", "http://www.agrodep.org/news?page=0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="views-row"]//span[@class="field-content"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="node__content"]/*|//div[@class="node__content"]//p//img')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "a", "em", "div", "strong"]:
