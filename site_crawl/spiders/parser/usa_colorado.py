...
class UsaColoradoParser(BaseParser):
    name = 'usa_colorado'
    
    # 站点id
    site_id = "fffd9fa2-0541-4cb1-b613-792bc780418f"
    # 站点名
    site_name = "科罗拉多州官网"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "fffd9fa2-0541-4cb1-b613-792bc780418f", "source_name": "科罗拉多州官网", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f0b64-fe6f-11ec-a30b-d4619d029786", "文章", "https://www.colorado.gov/articles?page=0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="text-2xl text-co-md-blue hover:underline"]')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('.//span[@class="field-content"]/a/@href').get()
                rsp_time = news_url.xpath('.//div[@class="views-field views-field-created"]/span/text()').get()
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//ul[@class="related-tags-article"]/li/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//article[@class="article full clearfix"]/div[2]/div[3]/*')
        if news_tags:
