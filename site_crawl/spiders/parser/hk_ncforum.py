...
class HkNcforumParser(BaseParser):
    name = 'ncforum'
    
    # 站点id
    site_id = "7348c15c-33e8-4ca3-ad7f-2bf4099ba4f3"
    # 站点名
    site_name = "新世纪论坛"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "7348c15c-33e8-4ca3-ad7f-2bf4099ba4f3", "source_name": "新世纪论坛", "direction": "hk", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("3980bd1b-4c41-346c-a343-38cdd91d3024", "新世纪论坛", "", "政治"),
            ("4a8cdad8-fe6f-11ec-a30b-d4619d029786", "新世纪论坛/新闻发布", "https://www.ncforum.org.hk/news?year=&page=1", "政治"),
            ("4a8cdc2c-fe6f-11ec-a30b-d4619d029786", "新世纪论坛/近期焦点", "https://www.ncforum.org.hk/foucs?year=&page=1", "政治"),
            ("4a8cdf06-fe6f-11ec-a30b-d4619d029786", "评论文章", "https://www.ncforum.org.hk/article", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="file-list"]//li')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('./a/@href').get()
                rsp_time = news_url.xpath('./span/text()').get()
                rsp_title = news_url.xpath('./a/text()').get()
                day, mt, year = re.findall('(\d{1,2})/(\d{1,2})/(\d{4})', rsp_time)[0]
                rsp_times = year + '-' + mt + '-' + day
                url = urljoin(response.url, rsp_url)
                self.Dict[url] = {'time': rsp_times, 'title': rsp_title}
                yield url
    ...
    def get_content_media(self, response) -> list:
        ...
        if ".pdf" in response.url:
            ...
            news_tags = response.xpath('//div[@class="article-content"]|//div[@class="article-content"]/*|'
                                       '//div[@class="article-content"]/p/a|//div[@class="article-content"]/p/img')
            if news_tags:
