...
class CnCeprParser(BaseParser):
    name = 'cepr'
    
    # 站点id
    site_id = "4d328174-6ca9-4284-bdbb-ba539c76ca21"
    # 站点名
    site_name = "经济政策研究中心"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "4d328174-6ca9-4284-bdbb-ba539c76ca21", "source_name": "经济政策研究中心", "direction": "cn", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8cfba8-fe6f-11ec-a30b-d4619d029786", "消息", "https://cepr.org/news?page=0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="c-card__title-link"]/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        time_ = response.xpath('//time[@class="c-single-header__meta-data"]/@datetime').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[contains(@class,"image-wrapper")]//img|'
                                   '//article/div[contains(@class,"c-text-block")]/*|'
                                   '//div[contains(@class,"content-wrapper")]/p')
        if news_tags:
