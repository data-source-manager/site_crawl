...
class IdTeriinParser(BaseParser):
    name = 'teriin'
    
    # 站点id
    site_id = "3e1f6b24-8850-4b35-8c86-60405fb06ed0"
    # 站点名
    site_name = "能源与资源研究所"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "3e1f6b24-8850-4b35-8c86-60405fb06ed0", "source_name": "能源与资源研究所", "direction": "id", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("9e1b9cf9-5243-3f36-8714-fee77ec0df0d", "新闻", "https://www.teriin.org/press-release?search_api_fulltext=&field_prelease_themes=All&field_pressrelease_date%5Bmin%5D=&field_pressrelease_date%5Bmax%5D=&page=0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="view-content"]/div/h3/a/@href').extract() or ""
        if news_urls:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//div[@class="field--items"]/div/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="block-region-main"]/div[contains(@class,"field--name-body")]/*|'
                                   '//div[@class="block-region-main"]/div[contains(@class,"field--type-image")]/img')
        if news_tags:
