...
class UsaUsniParser(BaseParser):
    name = 'usni'
    
    # 站点id
    site_id = "19a64356-c256-4d8f-95b6-91485f3e27b2"
    # 站点名
    site_name = "美国海军协会新闻"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "19a64356-c256-4d8f-95b6-91485f3e27b2", "source_name": "美国海军协会新闻", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8b69b4-fe6f-11ec-a30b-d4619d029786", "头条新闻", "https://news.usni.org/topstories", "政治"),
            ("9df6ed58-ac11-11ed-8769-1094bbebe6dc", "美国海军新闻", "https://news.usni.org/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h1[@class="entry-title"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="entry-content"]/*|//div[contains(@id,"attachment")]//img')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag == "div":
                        div_id = news_tag.xpath("./@id").extract_first()
                        if div_id and "attachment" in div_id:
    ...
    def parse_single_img(self, response, news_tag):
        ...
        if check_img(img_src):
            ...
            des = "".join(news_tag.xpath('.//p//text()').extract()).strip()
            if not des:
