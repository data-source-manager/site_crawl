...
class InIrppParser(BaseParser):
    name = 'irpp'

    # 站点id
    site_id = "fe822b7d-94c8-48c3-acae-a461dcf77efd"
    # 站点名
    site_name = "公共政策研究所"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "fe822b7d-94c8-48c3-acae-a461dcf77efd", "source_name": "公共政策研究所", "direction": "in", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块主题)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f3f76-fe6f-11ec-a30b-d4619d029786", "新闻", "https://irpp.org/media/page/1/?media_type=news-release", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//article/a[@class="article__link"]')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('./@href').get()
                rsp_time = news_url.xpath('.//span[@class="date-subtitle"]/text()').get()
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="media-single__content"]/*')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "strong", "em", "sup", "a"]:
