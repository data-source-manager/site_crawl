...
class TwMoeaParser(BaseParser):
    name = 'moea'
    
    # 站点id
    site_id = "a91b9d29-c640-406d-ba41-6ebd9d945a93"
    # 站点名
    site_name = "中华民国经济部全球咨询网"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "a91b9d29-c640-406d-ba41-6ebd9d945a93", "source_name": "中华民国经济部全球咨询网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("793cd881-1b75-4f25-8645-1d3cdbb2124c", "即時新聞澄清", "https://www.moea.gov.tw/Mns/populace/news/News.aspx?kind=9&menu_id=22333", "政治"),
            ("e700a380-7607-11ed-ad4d-d4619d029786", "本部新闻", "https://www.moea.gov.tw/Mns/populace/news/News.aspx?kind=1&menu_id=40", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[contains(@id,"nkTitle")]/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        ...
        """
        time_ = response.xpath('//div[@class="div-begin-date"]/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="div-left-info"]/*|'
                                   '//img[@class="img-responsive"]|'
                                   '//div[@class="div-left-info"]/text()')
        if news_tags:
