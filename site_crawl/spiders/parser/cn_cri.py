...
class CnCriParser(BaseParser):
    name = 'cri'
    
    # 站点id
    site_id = "10d26d9a-f7ff-4c51-a97b-e1945cedbaef"
    # 站点名
    site_name = "中国国际广播电台"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "10d26d9a-f7ff-4c51-a97b-e1945cedbaef", "source_name": "中国国际广播电台", "direction": "cn", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8cfedc-fe6f-11ec-a30b-d4619d029786", "国际锐评", "http://news.cri.cn/guojiruiping", "政治"),
            ("4a8cff4a-fe6f-11ec-a30b-d4619d029786", "滚动", "http://news.cri.cn/roll", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="sftx-list-text"]/a/@href|'
                                   '//div[@class="list-title"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@id="atitle"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        time_ = response.xpath('//span[@id="apublishtime"]/text()').extract_first()
        if time_:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//div[@class="list-label"]/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@id="abody"]/*|//div[@class="list-abody abody"]/p/img|'
                                   '//div[@id="abody"]/p/video')
        if news_tags:
