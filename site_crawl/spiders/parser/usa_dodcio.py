...
class UsaDoDcioParser(BaseParser):
    name = 'dodcio'
    
    # 站点id
    site_id = "67a01000-92ed-c847-2597-418ac5cb3b59"
    # 站点名
    site_name = "联合人工智能中心"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "67a01000-92ed-c847-2597-418ac5cb3b59", "source_name": "联合人工智能中心", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d2b32-fe6f-11ec-a30b-d4619d029786", "新闻", "https://dodcio.defense.gov/In-the-News/?Page=1", "政治"),
        ]
    ]
    ...
    def get_author(self, response) -> list:
        authors = response.xpath('//span[@class="line"]/text()').extract()
        return [a.strip().strip("By ") for a in authors]

    def get_pub_time(self, response) -> str:
        try:
            _time = response.xpath('//div[@class="category-date"]/text()').extract()
            print(_time[1])
            m, d, y = _time[1].strip(" | ").strip().strip("\n").split(" ")
            ...
            return f"{y}-{transf_month(m.strip('.'))}-{d.strip(',')} 00:00:00"
        except:
