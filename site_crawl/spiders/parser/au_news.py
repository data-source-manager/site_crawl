...
class AuNewsParser(BaseParser):
    name = 'news'
    
    # 站点id
    site_id = "d33ff8c3-6a21-451b-061b-04bb15861b55"
    # 站点名
    site_name = "澳洲新闻网"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "d33ff8c3-6a21-451b-061b-04bb15861b55", "source_name": "澳洲新闻网", "direction": "au", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f8422-fe6f-11ec-a30b-d4619d029786", "世界", "https://www.news.com.au/world/world-news", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h4[@class="storyblock_title"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@id="story-headline"]/text()').extract_first(default="")
        return title.strip() if title else ""

    def get_author(self, response) -> list:
        author_list = []

        authors = response.xpath('//meta[@name="article:author"]/@content').extract()
        if authors:
            for au in authors:
                if "-" in au:
                    au = au.split("-")[0]
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@id="story-primary"]//p|'
                                   '//div[@id="story-primary"]//div[contains(@class,"image")]//img')
        if news_tags:
