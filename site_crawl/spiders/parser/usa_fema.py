...
class UsaFemaParser(BaseParser):
    name = 'fema'
    
    # 站点id
    site_id = "13a3d852-97f3-d92f-d558-bcea8e1f9241"
    # 站点名
    site_name = "美国联邦紧急事务管理署"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "13a3d852-97f3-d92f-d558-bcea8e1f9241", "source_name": "美国联邦紧急事务管理署", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d3d70-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://www.fema.gov/about/news-multimedia/press-releases?combine=&field_related_locations_target_id=All&page=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//span[@class="field-content list-view-title"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        time_ = response.xpath('//time[@class="datetime"]/text()').extract_first()
        if time_:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//div[@class="fieldset-wrapper"]/div/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath(
            '//div[contains(@class,"text-formatted")]/p|//div[contains(@class,"wrapper--pdf")]/a')
        if news_tags:
