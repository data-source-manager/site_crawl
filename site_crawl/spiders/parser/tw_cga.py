...
class TwCgaParser(BaseParser):
    name = 'cga'
    
    # 站点id
    site_id = "cef6ec49-0082-430b-9cfc-52be033d3edb"
    # 站点名
    site_name = "行政院海岸巡防署全球资讯网"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "cef6ec49-0082-430b-9cfc-52be033d3edb", "source_name": "行政院海岸巡防署全球资讯网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8cb224-fe6f-11ec-a30b-d4619d029786", "海巡新闻", "https://www.cga.gov.tw/GipOpen/wSite/lp?ctNode=650&mp=999", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="news-list"]')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('./@href').get()
                rsp_times = news_url.xpath('./span[@class="date"]/text()').get()
                year, mt, day = re.findall('(\d{3})/(\d{1,2})/(\d{2})', rsp_times)[0]
                rsp_time = translatetTime.twYearDict[year] + '-' + mt + '-' + day
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="image"]/img|//div[@class="cp commoncpxsl"]/div[3]|'
                                   '//div[@class="download"]/ul/li/a|//div[@class="cp commoncpxsl"]/div[2]')
        if news_tags:
