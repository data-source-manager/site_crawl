...
class BdBidsParser(BaseParser):
    name = 'bids'
    
    # 站点id
    site_id = "9d067bdb-fb1b-43a2-8472-f892ecf2ddd4"
    # 站点名
    site_name = "孟加拉国发展研究所"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "9d067bdb-fb1b-43a2-8472-f892ecf2ddd4", "source_name": "孟加拉国发展研究所", "direction": "bd", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("b697e00a-dce6-43a2-b255-c1ddc884db5b", "News", "https://bids.org.bd/content.php?pg=1&ipp=10&url=page/news-events/&type=news", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="blog-post"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="blog-post-details"]/h2/text()').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="blog-post-details"]/img|//div[@class="blog-post-details"]/p')
        if news_tags:
