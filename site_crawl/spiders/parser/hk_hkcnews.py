...
class HkHkcnewsParser(BaseParser):
    name = 'hkcnews'
    
    # 站点id
    site_id = "d8063da4-1a36-438b-8ebe-6e5ab72694ae"
    # 站点名
    site_name = "众新闻"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "d8063da4-1a36-438b-8ebe-6e5ab72694ae", "source_name": "众新闻", "direction": "hk", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("e700c07c-7607-11ed-ad4d-d4619d029786", "政治", "https://www.hkcnews.com/data/newsposts/12?page=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        for text in json.loads(response.text).get("items", []):
            news_url = re.search(r"href\=\"(.+?)\"", text).groups()[0]
    ...
    def get_pub_time(self, response) -> str:
        try:
            _time = response.xpath('//div[@class="article-info"]/text()').extract_first()
            if len(_time.split("|")) == 4:
                return _time.split("|")[-1].strip()
            else:
                d, m, y = _time.split("|")[-1].split(":")[-1].strip().split(".")
                return f'20{y}-{m}-{d} 00:00:00'
        except:
    ...
    def get_content_media(self, response) -> list:
        content = []
        news_tags = response.xpath('//div[@class="article-content"]//p'
                                   '|//div[@class="article-content"]//img') or []
