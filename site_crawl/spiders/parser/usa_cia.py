...
class UsaCiaParser(BaseParser):
    name = 'cia'
    
    # 站点id
    site_id = "2ca81e74-8b6e-45ec-aa2d-cc640c813d3f"
    # 站点名
    site_name = "中央情报局"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "2ca81e74-8b6e-45ec-aa2d-cc640c813d3f", "source_name": "中央情报局", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("ac41abfa-a928-11ed-a6f8-1094bbebe6dc", "新闻与故事", "https://www.cia.gov/stories/", "政治"),
            ("e3bdb19f-fd13-3acc-8a99-f0fd9285066c", "资源", "https://www.cia.gov/resources/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//main[@id="main-content"]/div[3]//a[@class="card-wrapper"]/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="hero-title"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        if "resources" in response.url:
            return "9999-01-01 00:00:00"
        else:
            time_ = response.xpath('//div[contains(@class,"col-sm-12")]/div/text()').extract_first()
            if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[contains(@class,"free-form-content")]/*|//a[@class="link-icon d-block"]')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "strong", "a"]:
