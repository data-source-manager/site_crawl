...
class KrTongilvoiceParser(BaseParser):
    name = 'tongilvoice'
    
    # 站点id
    site_id = "dd72b188-a9ce-7a96-f7bf-696225f65eda"
    # 站点名
    site_name = "统之声"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "dd72b188-a9ce-7a96-f7bf-696225f65eda", "source_name": "统之声", "direction": "kr", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8dd456-fe6f-11ec-a30b-d4619d029786", "重大新闻", "http://www.tongilvoice.com/index.php?ptype=special&pagenum=1", "政治"),
            ("4a8dd4b0-fe6f-11ec-a30b-d4619d029786", "革命活动", "http://www.tongilvoice.com/index.php?ptype=revolution", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        if "special" in response.url:
            news_urls = response.xpath('//div[@class="gisa_con"]')
            if news_urls:
                for news_url in news_urls:
                    rsp_url = news_url.xpath('.//a[@class="title"]/@href').get()
                    rsp_title = news_url.xpath('.//a[@class="title"]/text()').get()
                    rsp_time = news_url.xpath('./div[@class="gisa_date"]/text()').get()
            ...
            if news_urls:
                for news_url in news_urls:
                    rsp_url = news_url.xpath('.//a[@class="revtitle"]/@href').get()
                    rsp_time = news_url.xpath('./div[@class="gisa_date"]/text()').get()
                    url = urljoin(response.url, rsp_url)
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@id="view_gisa"]/p')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "strong", "nobr"]:
