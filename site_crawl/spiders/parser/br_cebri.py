...
class BrCebriParser(BaseParser):
    name = 'cebri'
    
    # 站点id
    site_id = "f901d803-d907-409e-9217-4879ce6f0979"
    # 站点名
    site_name = "巴西国际关系中心"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "f901d803-d907-409e-9217-4879ce6f0979", "source_name": "巴西国际关系中心", "direction": "br", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("b2b349e6-49cc-4bda-97a6-cbb86cf503c0", "研究", "https://www.cebri.org/br/docs?tipo_id=6", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="card card-doc h-100"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        ...
        times_ = response.xpath(
            '//ul[@class="list-inline"]/li[@class="list-inline-item h6 cinza_tit"]/text()').extract_first()
        day, mt, year = re.findall('(\d{1,2}) (.*) (\d{4})', times_)[0]
        time_ = year + '-' + translatetTime.brDict[mt] + '-' + day
        if time_:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//ul[@class="list-inline"]/li[@class="list-inline-item card_tag"]/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="row"]//div[@class="texto"]/p|//div[@class="download text-center"]/a|'
                                   '//div[@class="download text-center"]//img')
        if news_tags:
