...
class JpMarubeniParser(BaseParser):
    name = 'marubeni'
    
    # 站点id
    site_id = "b68d9b83-f89e-e930-106f-d45039cf4ed1"
    # 站点名
    site_name = "丸红经济研究所"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "b68d9b83-f89e-e930-106f-d45039cf4ed1", "source_name": "丸红经济研究所", "direction": "jp", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8e0070-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://www.marubeni.com/en/news/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//li')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('./span[@class="text"]/a/@href').get()
                rsp_time = news_url.xpath('./span[@class="date"]/text()').get()
                rsp_title = news_url.xpath('./span[@class="text"]/a/text()').get()
    ...
    def get_content_media(self, response) -> list:
        ...
        if ".pdf" in response.url:
            ...
            news_tags = response.xpath('//figure[@class="image"]/img|//div[@class="articleDetailBox"]/*|'
                                       '//div[@class="reportList clr"]/p/img|//div[@class="reportList clr"]/ul/li/a|'
                                       '//div[@class="reportList clr"]/ul/ul//img|//section/div[@class="inner"]/div/p/a|'
                                       '//section/div[@class="inner"]/*|//div[@class="boxBorder"]/ul/li/a|'
                                       '//div[@class="btnWrap irGroup"]/p/a')
            if news_tags:
