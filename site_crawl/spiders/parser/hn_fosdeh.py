...
class HnFosDehParser(BaseParser):
    name = 'hn_fosdeh'
    
    # 站点id
    site_id = "7aacc240-a5cf-4749-82f3-83776f9b1bd9"
    # 站点名
    site_name = "洪都拉斯社会外债与发展论坛"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "7aacc240-a5cf-4749-82f3-83776f9b1bd9", "source_name": "洪都拉斯社会外债与发展论坛", "direction": "hn", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f2a0e-fe6f-11ec-a30b-d4619d029786", "发声", "https://fosdeh.com/category/pronunciamientos/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="archive-post-wrap"]//h2/a/@href').extract() or ""
        if news_urls:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="entry-content"]/*|'
                                   '//div[@class="entry-content"]//a[@class="wp-block-file__button"]')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "strong", "p"]:
