...
class SvFusadesParser(BaseParser):
    name = 'fusades'
    
    # 站点id
    site_id = "dfab8d61-4c2c-4dea-ad37-1a492e70a4de"
    # 站点名
    site_name = "萨尔瓦多经济与社会发展基金会"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "dfab8d61-4c2c-4dea-ad37-1a492e70a4de", "source_name": "萨尔瓦多经济与社会发展基金会", "direction": "sv", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d8e2e-fe6f-11ec-a30b-d4619d029786", "新闻", "http://fusades.org/?page=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//figure/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="title-post-head"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        times_ = response.xpath('//div[@class="author-date"]/span/text()').extract_first()
        day, month, year = re.findall(', (\d{1,2}) de (.*) del (\d{4})', times_)[0]
        time_ = year + '-' + translatetTime.itDict[month] + '-' + day
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="post-content"]/*|//div[@class="post-content"]//a')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "blockquote", "b"]:
