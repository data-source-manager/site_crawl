...
class UkIfsParser(BaseParser):
    name = 'uk_ifs'
    
    # 站点id
    site_id = "15c58a2c-804d-4446-881d-ab54a420ce64"
    # 站点名
    site_name = "财政研究所"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "15c58a2c-804d-4446-881d-ab54a420ce64", "source_name": "财政研究所", "direction": "uk", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f2590-fe6f-11ec-a30b-d4619d029786", "新闻", "https://ifs.org.uk/news", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="c-card__content-wrapper"]')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('./h3[@class="c-card__title"]/a/@href').get()
                rsp_time = news_url.xpath('./div/p/text()').get()
    ...
    def get_author(self, response) -> list:
        ...
        authors = response.xpath('//li[@class="c-single-header__meta-list-item"]/a/text()').extract()
        if authors:
            for au in authors:
                if au.strip():
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//ul[@class="c-single-header__meta-taxonomy-list"]/li/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath(
            '//div[@class="c-single-header__featured-image"]//picture/img[@alt="Sonya Krutikova"]|'
            '//div[@class="c-text-block__inner o-content-from-editor js-wysiwyg"]/*|'
            '//div[@class="ytp-title-text"]/a')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "a", "p"]:
