...
class HkCuhkParser(BaseParser):
    name = 'cuhk'
    
    # 站点id
    site_id = "231f835c-387e-4ffb-9b7a-3d67b314193f"
    # 站点名
    site_name = "新闻与传播学院"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "231f835c-387e-4ffb-9b7a-3d67b314193f", "source_name": "新闻与传播学院", "direction": "hk", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("9208f8d3-4fed-44c0-912a-836bb296ea64", "媒体报道", "https://www.com.cuhk.edu.hk/zh-TW/news/media-coverage/year.listevents/2022/04/26/-", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="ev_link_row"]/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h2[@class="contentheading"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//td[@class="ev_detail repeat"]/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//tr[@valign="top"]/td/*|//tr[@valign="top"]/td/p/img')
        if news_tags:
