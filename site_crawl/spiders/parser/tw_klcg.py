...
class TwKlcgParser(BaseParser):
    name = 'klcg'
    
    # 站点id
    site_id = "9a750d17-6ef9-43a3-b757-f3863e7afc8e"
    # 站点名
    site_name = "基隆市政府全球资讯网"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "9a750d17-6ef9-43a3-b757-f3863e7afc8e", "source_name": "基隆市政府全球资讯网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("e700e61a-7607-11ed-ad4d-d4619d029786", "市政新闻", "https://www.klcg.gov.tw/tw/News/NewsList", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="list_title"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//section[@class="cp"]/*|//div[contains(@class,"img-container")]/a/img')
        if news_tags:
    ...
    def get_read_count(self, response) -> int:
        read_count = 0
        read_count_str = response.xpath('//div[@class="publish_info"]/ul/li[4]/text()').get()
        if read_count_str:
