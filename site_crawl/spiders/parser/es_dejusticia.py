...
class EsDejusticiaParser(BaseParser):
    name = 'dejusticia'
    
    # 站点id
    site_id = "f8cfd9c6-7167-4f2c-9240-118a958097e2"
    # 站点名
    site_name = "法律·公正·社会"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "f8cfd9c6-7167-4f2c-9240-118a958097e2", "source_name": "法律·公正·社会", "direction": "es", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d8cc6-fe6f-11ec-a30b-d4619d029786", "出版物", "https://www.dejusticia.org/como-trabajamos/publicaciones/publicaciones-serie-dejusticia/", "政治"),
        ]
    ]
    ...
    def get_author(self, response) -> list:
        authors = response.xpath('//span[@class="author-name"]/text()').extract()
        return [a.strip() for a in authors]

    def get_pub_time(self, response) -> str:
        try:
            _time = "".join(response.xpath('//div[@class="uabb-subheading uabb-text-editor"]/p/text()').extract())
            m, d, y = _time.split("|")[-1].strip().split(" ")
    ...
    def get_content_media(self, response) -> list:
        content = []
        news_tags = response.xpath('//div[@class="fl-rich-text"]/*'
                                   '') or []
