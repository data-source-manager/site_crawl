...
class CnPeoPleParser(BaseParser):
    name = 'people'
    
    # 站点id
    site_id = "44cc819e-2401-4bc5-b140-325e1119ff24"
    # 站点名
    site_name = "人民网日语版"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "44cc819e-2401-4bc5-b140-325e1119ff24", "source_name": "人民网日语版", "direction": "cn", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8cfb30-fe6f-11ec-a30b-d4619d029786", "时政", "http://japan.people.com.cn/GB/35469/index.html", "政治"),
            ("4a8cfb6c-fe6f-11ec-a30b-d4619d029786", "财经", "http://japan.people.com.cn/GB/35463/index.html", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//ul[@class="list_16 mt10"]/li/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="clearfix w1000_320 text_title"]/h1/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        times_ = response.xpath('//div[@class="box01"]/div/text()').extract_first()
        year, mt, day, tm = re.findall('(\d{4})年(\d{2})月(\d{2})日(.*)  来源：', times_)[0]
        time_ = year + '-' + mt + '-' + day + ' ' + tm
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="box_con"]/p/text()')
        if news_tags:
