...
class TwFsrParser(BaseParser):
    name = 'fsr'
    
    # 站点id
    site_id = "09189786-d7a1-4091-97a0-f5f18cd5eaeb"
    # 站点名
    site_name = "台湾山水民调研究股份有限公司"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "09189786-d7a1-4091-97a0-f5f18cd5eaeb", "source_name": "台湾山水民调研究股份有限公司", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8cad10-fe6f-11ec-a30b-d4619d029786", "舆情分析", "https://www.fsr.com.tw/#dynamic", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="box"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//p[@class="ti"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//span[@class="day"]/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath(
            '//div[@class="newpic center-block"]/img|//div[@class="container"]/p[@class="mt1"]/text()')
        if news_tags:
