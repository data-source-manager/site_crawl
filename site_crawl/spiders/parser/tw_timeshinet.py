...
class TimeshinetParser(BaseParser):
    name = 'timeshinet'
    
    # 站点id
    site_id = "5c078745-2184-44cc-a414-d062f460fc7a"
    # 站点名
    site_name = "HINet"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "5c078745-2184-44cc-a414-d062f460fc7a", "source_name": "HINet", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("9123430e-2f72-11ed-a768-d4619d029786", "国际", "https://times.hinet.net/category/800", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath(
            '//div[@class="recentlyBox"]/ul//h4/a/@href').extract() or []
        if news_urls:
            for news_url in list(set(news_urls)):
                news_url = urljoin(response.url, news_url)
                if 'topicCategory' in news_url:
                    continue
                yield news_url
    ...
    def get_pub_time(self, response) -> str:
        # 16.07.2022 – 23:42
        pub = response.xpath('//div[@class="cpInfo"]/span/text()').extract_first()
        if pub:
            pub = pub.strip().split('(')[0]
