...
class UsaCbpParser(BaseParser):
    name = 'cbp'
    # 站点id
    site_id = "4c657749-8af1-99c1-1d16-1c51cb8ffe70"
    # 站点名
    site_name = "美国海关及边境保卫局"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "4c657749-8af1-99c1-1d16-1c51cb8ffe70", "source_name": "美国海关及边境保卫局", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d2a74-fe6f-11ec-a30b-d4619d029786", "媒体发布", "https://www.cbp.gov/newsroom/media-releases/all?page=0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//ul[@class="undecorated"]//a/@href').extract() or []
    ...
    def get_author(self, response) -> list:
        authors = response.xpath('//span[@class="field-content"]/a/text()').extract()
    ...
    def get_tags(self, response) -> list:
        tags = response.xpath('//div[@class="grid-row"]/div/a/text()').extract()
