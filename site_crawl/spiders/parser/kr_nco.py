...
class KrNcoParser(BaseParser):
    name = 'nco'
    
    # 站点id
    site_id = "b1814ab4-c667-4cbe-a1b1-062f61a66fd4"
    # 站点名
    site_name = "陆军士官学校"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "b1814ab4-c667-4cbe-a1b1-062f61a66fd4", "source_name": "陆军士官学校", "direction": "kr", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8eb01a-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://www.nco.mil.kr:456/nco/2310/subview.do", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//li[contains(@class,"thumbLi")]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h2[@class="artclViewTitle"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//div[@class="right"]/dl[2]/dd/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="artclAtchImg"]/img|//div[@class="artclView"]')
        if news_tags:
    ...
    def get_read_count(self, response) -> int:
        read_count = 0
        read_count_str = response.xpath('//div[@class="right"]/dl[1]/dd/text()').get()
        if read_count_str:
