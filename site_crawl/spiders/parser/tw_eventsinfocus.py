...
class TwEventSinFocusParser(BaseParser):
    name = 'tw_eventsinfocus'
    
    # 站点id
    site_id = "521aab1f-2a40-404a-ad34-a6384d4cc103"
    # 站点名
    site_name = "焦点事件"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "521aab1f-2a40-404a-ad34-a6384d4cc103", "source_name": "焦点事件", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8ef2f0-fe6f-11ec-a30b-d4619d029786", "焦点报导", "https://www.eventsinfocus.org/news?page=0", "政治"),
        ]
    ]
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//ul[@class="links field__items"]/li/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//picture/img|//div[@class="node__content clearfix"]/div[3]/*|'
                                   '//iframe[contains(@src,"youtube")]')
        if news_tags:
            for news_tag in news_tags:
                if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "a", "sup"]:
