...
class ItFeemParser(BaseParser):
    name = 'feem'
    
    # 站点id
    site_id = "8ed23477-da41-4762-8900-a027d89ab64e"
    # 站点名
    site_name = "埃尼恩里科·马特艾基金会"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "8ed23477-da41-4762-8900-a027d89ab64e", "source_name": "埃尼恩里科·马特艾基金会", "direction": "it", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8cf176-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://www.feem.it/en/press_area/press_releases/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="wpb_wrapper"]/p/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="titolo"]/text()').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="contenuto"]/*|//div[@class="wpb_wrapper"]/p/a')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "b", "i"]:
