...
class UsaResearchParser(BaseParser):
    name = 'usa_research'
    
    # 站点id
    site_id = "fba577ae-5d0d-49f3-a20f-f6b96908c256"
    # 站点名
    site_name = "社会经济研究中心"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "fba577ae-5d0d-49f3-a20f-f6b96908c256", "source_name": "社会经济研究中心", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f20cc-fe6f-11ec-a30b-d4619d029786", "新闻", "https://www.case-research.eu/en/news?&page=0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="aclick"]/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="thtitle-post"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        """
        time_ = response.xpath('//div[@class="pull-left date"]/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="list_news_content"]/*')
        if news_tags:
