...
class TwHikingParser(BaseParser):
    name = 'hiking'
    
    # 站点id
    site_id = "a4f23fee-925c-46fd-8e15-b19c7aab0162"
    # 站点名
    site_name = "健行笔记"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "a4f23fee-925c-46fd-8e15-b19c7aab0162", "source_name": "健行笔记", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块主题)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("e7007d10-7607-11ed-ad4d-d4619d029786", "新闻", "https://hiking.biji.co/index.php?q=news&label=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="text-current"]/@href').extract() or ""
        if news_urls:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="fr-view"]/*')
        if news_tags:
                ...
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p"]:
                        if news_tag.xpath('.//iframe'):
                            media_dict = self.parse_media(response, news_tag, media_type="mp4")
                            if media_dict:
    ...
    def parse_single_img(self, response, news_tag):
        ...
        img_src = news_tag.xpath(".//img/@data-src").extract_first()
        if check_img(img_src):
            img_url = urljoin(response.url, img_src)
            des = "".join(news_tag.xpath('.//span//text()').extract()).strip()
            if not des:
    ...
    def parse_media(self, response, news_tag, media_type):
        videoUrl = news_tag.xpath(".//iframe/@data-src").extract_first()
        suffix = f".{media_type}"
