...
class TwHccgParser(BaseParser):
    name = 'hccg'
    
    # 站点id
    site_id = "abe316f3-3687-4876-8e53-107f6dbc7cd6"
    # 站点名
    site_name = "新竹市政府全球资讯网"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "abe316f3-3687-4876-8e53-107f6dbc7cd6", "source_name": "新竹市政府全球资讯网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("e700d526-7607-11ed-ad4d-d4619d029786", "市政新闻", "https://www.hccg.gov.tw/ch/home.jsp?id=30122&parentpath=&mcustomize=&serno=null&page=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="css_td list_title"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="content_detail_img_news"]//img|//div[@class="main-a_01"]/*')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "a", "div"]:
    ...
    def get_read_count(self, response) -> int:
        read_count = 0
        read_count_str = response.xpath('//div[@id="home_update"]/text()').get()
        if read_count_str:
            read_count = int(re.findall('瀏覽人次：(.*) 人', read_count_str.strip())[0])
        return read_count
