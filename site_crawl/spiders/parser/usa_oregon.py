...
class OregonParser(BaseParser):
    name = 'oregon'
    
    # 站点id
    site_id = "ad4e6358-476a-4bb8-b0ce-773ce710a9d3"
    # 站点名
    site_name = "俄勒冈陆军国民警卫队"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "ad4e6358-476a-4bb8-b0ce-773ce710a9d3", "source_name": "俄勒冈陆军国民警卫队", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("91232cac-2f72-11ed-a768-d4619d029786", "新闻稿", "https://www.oregon.gov/newsroom/Pages/newsroom.aspx", "军事"),
        ]
    ]
    ...
    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="or-newsroom-article-title"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        # 16.07.2022 – 23:42
        pub = response.xpath('//div[@class="articledate"]/text()').extract_first()
        if pub:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath(
            '//div[@class="or-newsroom-article-body-full"]')
        if news_tags:
    ...
    def parse_text(self, news_tag):
        ...
        if cons:
            for x in cons:
                if x.strip():
                    res_dicts.append({'data': x.strip(), 'type': 'text'})
        return res_dicts
