...
class UkForumFortheFutureParser(BaseParser):
    name = 'forumforthefuture'
    
    # 站点id
    site_id = "7389f175-a286-c670-54cb-3cd1e45dc381"
    # 站点名
    site_name = "未来论坛"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "7389f175-a286-c670-54cb-3cd1e45dc381", "source_name": "未来论坛", "direction": "uk", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f89a4-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://www.forumforthefuture.org/press-releases", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="content postContent pageContent"]//li/a/@href').extract() or []
        for news_url in list(set(news_urls)):
            if news_url.startswith('https://www.forumforthefuture.org/') and "Download.ashx" not in news_url:
    ...
    def get_content_media(self, response) -> list:
        content = []
        news_tags = response.xpath('//ul[@id="carouselSlides"]//img'
                                   '|//div[@class="content postContent pageContent"]/*'
                                   '|//div[@class="content postContent pageContent"]//img') or []
