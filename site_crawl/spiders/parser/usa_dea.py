...
class UsaDeaParser(BaseParser):
    name = 'dea'
    
    # 站点id
    site_id = "9fa441ae-ebde-da60-d12e-066009cd285e"
    # 站点名
    site_name = "禁毒缉私局"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "9fa441ae-ebde-da60-d12e-066009cd285e", "source_name": "禁毒缉私局", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d2ad8-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://www.dea.gov/what-we-do/news/press-releases?keywords=&page=0", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="l-view__content "]//h3/a/@href').extract() or []
    ...
    def get_author(self, response) -> list:
        authors = response.xpath('//div[@class="press__contact"]/text()').extract()
        return [a.strip() for a in authors]

    def get_pub_time(self, response) -> str:
        # March 27, 2023
        _time = response.xpath('//div[@class="press__date"]/text()').extract_first()
        if _time:
            time_ = datetime.strptime(_time, "%B %d, %Y")
            return parseTimeWithOutTimeZone(time_, site_name="禁毒缉私局")
    ...
    def get_content_media(self, response) -> list:
        content = []
        news_tags = response.xpath('//div[@class="wysiwyg"]/*'
                                   '|//div[@class="featured-image"]//img') or []
