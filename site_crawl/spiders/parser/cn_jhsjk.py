...
class CnJhsJkParser(BaseParser):
    name = 'jhsjk'
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="news-box"]/div[2]/ul/li/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="d2txt clearfix"]/h1/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        times_ = response.xpath('//div[@class="d2txt_1 clearfix"]/text()').extract_first()
        time_ = re.findall('发布时间：(.*)', times_)[0]
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="d2txt_con clearfix"]/p')
        if news_tags:
