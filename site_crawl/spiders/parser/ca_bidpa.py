...
class CaBidpaParser(BaseParser):
    name = 'bidpa'
    
    # 站点id
    site_id = "6c13a83f-0f44-49b8-b206-818c2a5de4ff"
    # 站点名
    site_name = "博茨瓦纳发展政策分析研究所"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "6c13a83f-0f44-49b8-b206-818c2a5de4ff", "source_name": "博茨瓦纳发展政策分析研究所", "direction": "ca", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f3a9e-fe6f-11ec-a30b-d4619d029786", "出版物", "https://bidpa.bw/publications/page/1/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="btn btn-xs btn-primary pt-right"]/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="page-title"]/text()').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//img[@class="owl-lazy img-responsive"]|'
                                   '//div[@class="entry-content"]//div[@class="wpb_wrapper"]/p')
        if news_tags:
