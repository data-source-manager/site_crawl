...
class TwMoigovParser(BaseParser):
    name = 'moigov'
    
    # 站点id
    site_id = "a1976496-9ecd-46ed-918f-536b9a8e163e"
    # 站点名
    site_name = "中华民国内政部全球资讯网"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "a1976496-9ecd-46ed-918f-536b9a8e163e", "source_name": "中华民国内政部全球资讯网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("f0e3a7cc-c48a-4930-bd0d-22e4f1e29ebf", "新闻发布", "https://www.moi.gov.tw/News.aspx?n=4&sms=9009&_CSN=1588", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//tbody//td[@class="CCMS_jGridView_td_Class_2 "]/span/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        ...
        times_ = response.xpath('//div[@class="group page-content "]//li[@data-index="1"]/span/text()').extract_first()
        year, mt, day, tm = re.findall('發布日期：(\d{3})-(\d{2})-(\d{2}) (.*)', times_)[0]
        time_ = translatetTime.twYearDict[year] + '-' + mt + '-' + day + ' ' + tm
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="essay"]/div[@class="p"]/*')
        if news_tags:
