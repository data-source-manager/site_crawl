...
class TwAfpcParser(BaseParser):
    name = 'afpc'
    
    # 站点id
    site_id = "b66aab79-102d-42c0-9407-720c897db479"
    # 站点名
    site_name = "宪兵指挥部全球资讯网"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "b66aab79-102d-42c0-9407-720c897db479", "source_name": "宪兵指挥部全球资讯网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("ebbc1701-c9f5-44b2-8dbe-916ae3610e7c", "最新新闻", "https://afpc.mnd.gov.tw/NewsRoom/News_List.aspx?ID=2", "政治"),
        ]
    ]
    ...
    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="NewsTitle"]/h2/span/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//div[@class="UpDated"]/p[1]/span/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[contains(@class,"swiper-slide")]/img|//div[@class="NewsDetail"]/*|'
                                   '//div[@class="NewsDetail"]/p/img')
        if news_tags:
