...
class GbInstItuteofwaterParser(BaseParser):
    name = 'instituteofwater'
    
    # 站点id
    site_id = "3ff66de1-91a7-3ee6-282c-c6d9ac94ad9c"
    # 站点名
    site_name = "水研究所"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "3ff66de1-91a7-3ee6-282c-c6d9ac94ad9c", "source_name": "水研究所", "direction": "gb", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d9310-fe6f-11ec-a30b-d4619d029786", "新闻", "https://www.instituteofwater.org.uk/news/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="more-link"]/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="title-post entry-title"]/text()').extract_first(default="")
        return title.strip() if title else ""

    def get_author(self, response) -> list:
        author_list = []

        authors = response.xpath('//span[@class="author vcard"]/a/text()').extract()
        if authors:
            for au in authors:
                if au.strip():
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//time[@class="entry-date published"]/@datetime').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="entry-thumb"]/img|//div[@class="entry-content"]/*|'
