...
class Kz_governmentParser(BaseParser):
    name = 'kz_government'
    
    # 站点id
    site_id = "b6d29839-8cd6-b45d-7cee-3f62bd616d40"
    # 站点名
    site_name = "哈萨克斯坦政府网"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "b6d29839-8cd6-b45d-7cee-3f62bd616d40", "source_name": "哈萨克斯坦政府网", "direction": "kz", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f9f84-fe6f-11ec-a30b-d4619d029786", "新闻", "http://www.government.kz/kz/news/pressa?page=1", "政治"),
        ]
    ]
    ...
    def get_pub_time(self, response) -> str:
        ...
        """
        time_tran_dict = a = {'Қаңтар': 'January', 'Ақпан': 'February', 'Наурыз': 'March', 'Сәуір': 'April',
                              'Мамыр': 'May', 'Маусым': 'June',
                              'Шілде': 'July', 'Тамыз': 'August', 'Қыркүйек': 'September', 'Қазан': 'October',
                              'Қараша': 'November', 'Желтоқсан': 'December'}
        time_ = response.xpath('//p[@class="article__date"]/text()').extract_first()
        time_list = []
        for t in time_.split('\n'):
            if t.strip():
                time_list.append(t.strip())
        time_str = ' '.join(time_list).strip()
        time_str = time_str.replace(time_str.split(' ')[1], time_tran_dict[time_str.split(' ')[1]])
        if time_str:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="articles__container"][2]/*')
        if news_tags:
