...
class UsaUscgParser(BaseParser):
    name = 'usa_uscg'
    
    # 站点id
    site_id = "556636cc-5b51-128f-73f0-d30236532714"
    # 站点名
    site_name = "美国海岸警卫队"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "556636cc-5b51-128f-73f0-d30236532714", "source_name": "美国海岸警卫队", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f0542-fe6f-11ec-a30b-d4619d029786", "新闻室", "https://www.news.uscg.mil/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="feedListAFTitle"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        ...
        times_ = response.xpath('//span[@class="dateline rs_skip"]/text()').extract_first()
        time_ = re.findall('at (.*) EDT', times_)[0]
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="bulletin_body"]/p')
        if news_tags:
