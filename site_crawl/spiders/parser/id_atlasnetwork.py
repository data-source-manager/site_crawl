...
class IdAtlAsnetworkParser(BaseParser):
    name = 'atlasnetwork'
    
    # 站点id
    site_id = "0dd811a6-37d3-408e-94e6-5eb1ae2e14cf"
    # 站点名
    site_name = "经济和社会现实研究中心"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "0dd811a6-37d3-408e-94e6-5eb1ae2e14cf", "source_name": "经济和社会现实研究中心", "direction": "id", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f44bc-fe6f-11ec-a30b-d4619d029786", "新闻", "https://www.atlasnetwork.org/articles/category/news/", "政治"),
        ]
    ]
    ...
    def get_pub_time(self, response) -> str:
        time_ = response.xpath('//dd/time/@datetime').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//figure[contains(@class,"o-container")]/img|'
                                   '//div[contains(@class,"components-article")]//p')
        if news_tags:
