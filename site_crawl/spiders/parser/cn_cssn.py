...
class CnCssnParser(BaseParser):
    name = 'cssn'
    
    # 站点id
    site_id = "085c8b57-150c-44d9-99dc-3f8e4deebfac"
    # 站点名
    site_name = "中国社科院日本研究所"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "085c8b57-150c-44d9-99dc-3f8e4deebfac", "source_name": "中国社科院日本研究所", "direction": "cn", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("d9bc3313-16c4-34e6-ac71-9bf90769bce3", "学术成果", "", "政治"),
            ("4a8cfe14-fe6f-11ec-a30b-d4619d029786", "学术成果/时评", "http://ijs.cssn.cn/xsyj/bkwz/", "政治"),
            ("3e78611a-159b-3302-aebc-be1b0985186b", "新闻资讯", "", "政治"),
            ("4a8cfd2e-fe6f-11ec-a30b-d4619d029786", "新闻资讯/本所新闻", "http://ijs.cssn.cn/xsdt/bsxw/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="right_list"]/ul/li')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('./a/@href').get()
                rsp_time = news_url.xpath('./span[1]/text()').get()
                rsp_author = news_url.xpath('./span[2]/text()').get()
                url = urljoin(response.url, rsp_url)
                self.Dict[url] = {'time': rsp_time, 'author': rsp_author}
                yield url

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="right_title"]/p/text()').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="TRS_Editor"]/*|//div[@class="TRS_Editor"]//img')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "div", "font"]:
