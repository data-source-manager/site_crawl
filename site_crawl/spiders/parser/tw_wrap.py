...
class TwWrapParser(BaseParser):
    name = 'wrap'
    # 站点id
    site_id = "c3354697-a6ba-4925-93c6-3bdc557292a4"
    # 站点名
    site_name = "经济部水利规划实验所"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "c3354697-a6ba-4925-93c6-3bdc557292a4", "source_name": "经济部水利规划实验所", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("cfd17693-204b-39f2-a61b-172ae06702a6", "发布消息", "", "政治"),
            ("4a8cb2ba-fe6f-11ec-a30b-d4619d029786", "发布消息/动态报道", "https://www.wrap.gov.tw/News.aspx?n=26383&sms=9833", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//td[@class="CCMS_jGridView_td_Class_0 "]/span/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        times_ = response.xpath('//div[@class="group page-content "]//li[@data-index="2"]/span/text()').extract_first()
        year, mt, day = re.findall('活動日期：(\d{3})-(\d{1,2})-(\d{1,2})', times_)[0]
        time_ = translatetTime.twYearDict[year] + '-' + mt + '-' + day
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="p"]/*|//div[@class="list-pic pic-download"]//img')
        if news_tags:
    ...
    def parseOnetext(self, news_tag) -> list:
        ...
        if cons:
            dic = {}
            oneCons = "".join(cons).strip().replace('\xa0', '').replace('\r', '').replace('\n', '')
            if oneCons:
                dic['data'] = oneCons
                dic['type'] = 'text'
                new_cons.append(dic)
