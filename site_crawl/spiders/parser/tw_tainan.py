# -*- coding: utf-8 -*-
# update:(liyun|2023-03-23) -> 日期解析修正
import re
...
class TwTainanParser(BaseParser):
    name = 'tainan'
    
    # 站点id
    site_id = "1c0f70c8-c192-48c1-82b2-a0d8c58a2c4f"
    # 站点名
    site_name = "台南市政府全球资讯网"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "1c0f70c8-c192-48c1-82b2-a0d8c58a2c4f", "source_name": "台南市政府全球资讯网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("e700adee-7607-11ed-ad4d-d4619d029786", "市府新闻", "https://www.tainan.gov.tw/News.aspx?n=13370&sms=9748", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="in"]/table//tr//a/@href').extract() or []
    ...
    def get_pub_time(self, response) -> str:
        try:
            _, y, m, d, hm = re.findall(r"發稿(時間|日期)\：(\d+)年(\d+)月(\d+)日([\d:]+)", response.text)[0]
            return f'{1911 + int(y)}-{m}-{d} {hm}:00'
        except:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//meta[@name="DC.Keywords"]/@content').extract()
        if tags:
            if "," in tags:
                tags = tags.split(",")
