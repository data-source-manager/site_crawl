...
class MoMacaupostdailyParser(BaseParser):
    name = 'macaupostdaily'
    
    # 站点id
    site_id = "a02dd8fe-2189-49f9-b41d-f8a38cba3bdd"
    # 站点名
    site_name = "澳门邮报"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "a02dd8fe-2189-49f9-b41d-f8a38cba3bdd", "source_name": "澳门邮报", "direction": "mo", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8cf3f6-fe6f-11ec-a30b-d4619d029786", "消息", "https://www.macaupostdaily.com/index.php", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="text"]/strong/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="art_view"]/h2/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//div[@class="info"]/p/em/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="art_cont"]//img|//div[@class="art_cont"]/*')
        if news_tags:
