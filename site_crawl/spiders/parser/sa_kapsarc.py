...
class SaKapsarcParser(BaseParser):
    name = 'kapsarc'
    
    # 站点id
    site_id = "951cec64-09c8-43eb-b6bd-0528b3acc624"
    # 站点名
    site_name = "阿卜杜拉石油研究中心"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "951cec64-09c8-43eb-b6bd-0528b3acc624", "source_name": "阿卜杜拉石油研究中心", "direction": "sa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d1110-fe6f-11ec-a30b-d4619d029786", "新闻与事件", "https://www.kapsarc.org/news-events/#news", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//footer[@class="card-footer"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="container"]/h1/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath(
            '//div[@class="single-page-summary"]//ul/li[2]/span[@class="summary-content"]/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="section-inner single-page-content"]/*|'
                                   '//div[@class="section-inner single-page-content"]/p/a')
        if news_tags:
