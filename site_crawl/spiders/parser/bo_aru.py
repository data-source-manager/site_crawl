...
class BoAruParser(BaseParser):
    name = 'bo_aru'
    
    # 站点id
    site_id = "fe4c3988-6859-40f3-9488-9f9648c3fb2a"
    # 站点名
    site_name = "阿鲁基金会"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "fe4c3988-6859-40f3-9488-9f9648c3fb2a", "source_name": "阿鲁基金会", "direction": "bo", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8ef75a-fe6f-11ec-a30b-d4619d029786", "消息", "https://www.aru.org.bo/noticias.php?page_no=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//li[@class="media py-3"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h5[@class="mt-0 mb-2 titulo-sin"]/b/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        """
        time_ = response.xpath('//i[@class="text-muted float-right"]/text()').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//a[@class="col-12 col-lg-8"]/img|//div[@class="media-body"]/p')
        if news_tags:
