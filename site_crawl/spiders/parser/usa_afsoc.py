...
class UsaAfsocParser(BaseParser):
    name = 'afsoc'
    
    # 站点id
    site_id = "2a5d3ec8-becd-fd88-43e2-823f652830bc"
    # 站点名
    site_name = "美国空军特种作战司令部"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "2a5d3ec8-becd-fd88-43e2-823f652830bc", "source_name": "美国空军特种作战司令部", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f0fc4-fe6f-11ec-a30b-d4619d029786", "新闻", "https://www.afsoc.af.mil/News/?Page=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="thumb"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//picture/img|//section[@class="article-detail-content"]/*')
        if news_tags:
