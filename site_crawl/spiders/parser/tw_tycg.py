...
class TwTycgParser(BaseParser):
    name = 'tycg'
    
    # 站点id
    site_id = "2a055671-96c8-4567-9833-cc74f660c0d6"
    # 站点名
    site_name = "桃园县政府入口网站"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "2a055671-96c8-4567-9833-cc74f660c0d6", "source_name": "桃园县政府入口网站", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("e7009a98-7607-11ed-ad4d-d4619d029786", "市政新闻", "https://www.tycg.gov.tw/ch/home.jsp?id=10412&parentpath=0%2C10401&mcustomize=", "政治"),
            ("e700f826-7607-11ed-ad4d-d4619d029786", "最新消息", "https://www.tycg.gov.tw/ch/home.jsp?id=10408&parentpath=0%2C10401&mcustomize=", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="list_a"]/@href|//div[@class="post-title"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//meta[@name="keywords"]/@content').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="news-info"]|//div[@class="news-info"]/img|'
                                   '//div[@class="news-content"]//li/a|//div[@class="news-photo"]/img|'
                                   '//div[@class="news-info"]/*')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "div", "strong"]:
    ...
    def get_read_count(self, response) -> int:
        read_count = 0
        read_count_str = response.xpath("//div[@class='visitor']//li[2]/text()").get()
        print(read_count_str)
        if read_count_str:
            read_count = int(re.findall('瀏覽人次：(.*) 人', read_count_str)[0])
        return read_count
