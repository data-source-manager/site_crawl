...
class UsaCybercomParser(BaseParser):
    name = 'cybercom'
    
    # 站点id
    site_id = "57927417-e2b6-4ba8-887a-12ae9122ccc1"
    # 站点名
    site_name = "网络司令部"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "57927417-e2b6-4ba8-887a-12ae9122ccc1", "source_name": "网络司令部", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("526d2fdb-1a37-3dc8-906e-fdafb4e89763", "媒体", "", "政治"),
            ("91236f78-2f72-11ed-a768-d4619d029786", "媒体/消息", "https://www.cybercom.mil/Media/News/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="more"]/@href').extract() or ""
        if news_urls:
    ...
    def get_content_media(self, response) -> list:
        ...
        if news_tags:
            for news_tag in news_tags:
                if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "div", "ul"]:
                    if news_tag.root.tag == "ul":
                        for x in news_tag.xpath('./li'):
                            news_tag = x
                            con_text = self.parse_text(x)
                            if con_text:
