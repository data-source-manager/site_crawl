...
class ThedialogueParser(BaseParser):
    name = 'thedialogue'
    
    # 站点id
    site_id = "d6c5a579-b706-45e2-86dc-6ac7d4eefc1c"
    # 站点名
    site_name = "美洲对话"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "d6c5a579-b706-45e2-86dc-6ac7d4eefc1c", "source_name": "美洲对话", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("91233620-2f72-11ed-a768-d4619d029786", "分析", "https://www.thedialogue.org/analysis/?s-type%5B%5D=25", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath(
            '//article[@class="teaser3"]//h3/a/@href').extract() or []
        if news_urls:
    ...
    def get_author(self, response) -> list:
        authors = response.xpath('//h3[contains(text(), "Author(s)")]/following-sibling::ul/li/a/text()').extract()
        if authors:
    ...
    def get_pub_time(self, response) -> str:
        # 16.07.2022 – 23:42
        pub = response.xpath('//header/p/time/text()').extract_first()
        if pub:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath(
            '//div[@class="page1"]/p|'
            '//div[@class="page1"]//span[@class="img1"]/img')
        if news_tags:
