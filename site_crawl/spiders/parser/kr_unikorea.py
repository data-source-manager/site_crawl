...
class KrUnikoreaParser(BaseParser):
    name = 'unikorea'
    
    # 站点id
    site_id = "7f688528-762b-43ad-84a7-60f349723132"
    # 站点名
    site_name = "韩国统一部"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "7f688528-762b-43ad-84a7-60f349723132", "source_name": "韩国统一部", "direction": "kr", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f2734-fe6f-11ec-a30b-d4619d029786", "现场新闻", "https://unikorea.go.kr/unikorea/news/live/?pageIdx=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//span[@class="text"]/a/@href').extract() or []
        for news_url in list(set(news_urls)):
            yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h3[@class="detail-title"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        try:
            _time = response.xpath('//dd[@class="created"]/text()').extract()[-1]
    ...
    def get_content_media(self, response) -> list:
        content = []
        news_tags = response.xpath('//div[@class="board_content thumbnail-view"]/*'
                                   '|//div[@class="board_content thumbnail-view"]//img') or []
