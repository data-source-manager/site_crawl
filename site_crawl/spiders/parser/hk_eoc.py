...
class HkEocParser(BaseParser):
    name = 'eoc'
    
    # 站点id
    site_id = "3582e55f-e7a2-1a1c-abad-99291ec00333"
    # 站点名
    site_name = "平等机会委员会"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "3582e55f-e7a2-1a1c-abad-99291ec00333", "source_name": "平等机会委员会", "direction": "hk", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("1e79376c-ff89-407f-b52f-916e7d186f2c", "新闻", "https://www.eoc.org.hk/en/PressRelease", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//li[@class="nav-item"]/a/@href').extract() or []
    ...
    def get_pub_time(self, response) -> str:
        _time = response.xpath('//div[@class="indent text-only"]/h3[2]/text()').extract_first()
    ...
    def get_content_media(self, response) -> list:
        content = []
        news_tags = response.xpath('//div[@class="indent text-only"]/div/*'
                                   '|//div[@class="indent text-only"]//img') or []
