...
class InIpcsParser(BaseParser):
    name = 'ipcs'
    
    # 站点id
    site_id = "24b898b4-fd50-4343-b6d8-a3d6b1fe3d1c"
    # 站点名
    site_name = "印度和平与冲突研究所"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "24b898b4-fd50-4343-b6d8-a3d6b1fe3d1c", "source_name": "印度和平与冲突研究所", "direction": "in", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d81b8-fe6f-11ec-a30b-d4619d029786", "评论", "http://www.ipcs.org/commentaries.php?pageno=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//li[@class="clearfix"]/a[1]/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="col-md-9"]/p[1]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        times_ = response.xpath('//div[@class="col-md-9"]/p[@style="font-size: 13px;"]/text()').extract_first()
        day, month, year = re.findall('(\d{1,2}) (.*), (\d{4})', times_)[0]
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//span[@class="avatar_img"]//img|//div[@class="text-div"]/*'
        ...
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "a", "b"]:
