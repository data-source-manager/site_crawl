...
class GbBristolParser(BaseParser):
    name = 'bristol'
    
    # 站点id
    site_id = "50b0e764-6f73-a657-d67c-4362e8fa3a2e"
    # 站点名
    site_name = "布里斯托大学卡伯特研究所"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "50b0e764-6f73-a657-d67c-4362e8fa3a2e", "source_name": "布里斯托大学卡伯特研究所", "direction": "gb", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d92a2-fe6f-11ec-a30b-d4619d029786", "新闻", "https://www.bristol.ac.uk/news/news-results.html?sort=NewsArticle-datePublished&filter%3ANewsArticle-sourceOrganization=UoB", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//li[contains(@class,"news-listing-story ")]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = response.xpath('//meta[@property="datePublished"]/@content').extract_first()
        if time_:
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="container-news-pics"]/img|//div[@property="articleBody"]/*|'
                                   '//div[@class="column grid_9 content-news-article"]/p|'
                                   '//div[@class="column grid_9 content-news-article"]')
        if news_tags:
