...
class VnCiemParser(BaseParser):
    name = 'ciem'
    
    # 站点id
    site_id = "29e8fd85-3696-4519-87ce-9f32695eddc5"
    # 站点名
    site_name = "越南中央经纪管理研究院"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "29e8fd85-3696-4519-87ce-9f32695eddc5", "source_name": "越南中央经纪管理研究院", "direction": "vn", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f8e18-fe6f-11ec-a30b-d4619d029786", "消息", "http://ciem.org.vn/danh-muc-tin-tuc/407/tin-tuc?page=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@id="content"]//h3/a/@href').extract() or []
    ...
    def get_pub_time(self, response) -> str:
        try:
            _time = response.xpath('//div[@class="col-md-9"]/p[1]/text()').extract()
            d, m, y = _time[1].strip("\r\n").strip().split("/")
    ...
    def get_content_media(self, response) -> list:
        content = []
        news_tags = response.xpath('//div[@class="ct text-justify"]//p/*') or []
