...
class InGatewayHouseParser(BaseParser):
    name = 'in_gatewayhouse'
    
    # 站点id
    site_id = "3b37c944-0f58-458f-979f-d49bf68107e4"
    # 站点名
    site_name = "梵门阁"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "3b37c944-0f58-458f-979f-d49bf68107e4", "source_name": "梵门阁", "direction": "in", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8dce16-fe6f-11ec-a30b-d4619d029786", "文章", "https://www.gatewayhouse.in/publications/articles/", "政治"),
        ]
    ]
    ...
    def get_author(self, response) -> list:
        ...
        authors = response.xpath('//div[@id="aboutauthor"]/p/span/a/text()').extract()
        if authors:
            for au in authors:
                if au.strip():
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//p[@id="tags"]/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="entry-content"]/*|//div[@class="entry-content"]/img')
        if news_tags:
