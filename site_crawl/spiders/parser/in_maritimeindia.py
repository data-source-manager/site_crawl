...
class InMariTimeIndiaParser(BaseParser):
    name = 'in_maritimeindia'
    
    # 站点id
    site_id = "8150f96e-7f88-3088-6cab-bdc7851cbab6"
    # 站点名
    site_name = "印度海上基金会"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "8150f96e-7f88-3088-6cab-bdc7851cbab6", "source_name": "印度海上基金会", "direction": "in", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("afd22421-d656-30ea-9080-07aeff055913", "出版物", "", "政治"),
            ("4a8e028c-fe6f-11ec-a30b-d4619d029786", "出版物/文章", "https://maritimeindia.org/category/articles-nmf/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//article[contains(@class,"slide-entry flex_column")]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h1[@class="post-title entry-title"]/a/text()').extract_first(default="")
        return title.strip() if title else ""

    def get_author(self, response) -> list:
        author_list = []

        authors = response.xpath('//span[@class="editor"]/text()').extract()
        if authors:
            for au in authors:
                if au.strip():
