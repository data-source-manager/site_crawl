...
class TwKanNewYorkParser(BaseParser):
    name = 'tw_kannewyork'
    
    # 站点id
    site_id = "c820d5b7-a656-4151-9452-c61b327f97cb"
    # 站点名
    site_name = "看纽约"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "c820d5b7-a656-4151-9452-c61b327f97cb", "source_name": "看纽约", "direction": "TW", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8de806-fe6f-11ec-a30b-d4619d029786", "新闻", "https://www.kannewyork.com/news/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//p[@class="indexNewsListMetaTitle"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="title"]/text()').extract_first(default="")
    ...
    def get_pub_time(self, response) -> str:
        ...
        time_ = re.findall('"datePublished": "(.*)",', json_data)[0]
        if time_:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//div[@class="text-right"]/span/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
