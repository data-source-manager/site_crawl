...
class TwJudicialParser(BaseParser):
    name = 'judicial'
    
    # 站点id
    site_id = "5613f78f-7003-40c6-9df2-2835f40cd3a3"
    # 站点名
    site_name = "司法院全球资讯网"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "5613f78f-7003-40c6-9df2-2835f40cd3a3", "source_name": "司法院全球资讯网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("dd22607b-dbaf-4a2c-aa32-aa0b0361852f", "全部新聞", "https://www.judicial.gov.tw/tw/lp-1790-1-1-20.html", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//td[@data-title="標題"]//a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h2[@class="pageTitle"]/text()').extract_first(default="")
        ...
        if news_tags:
            for news_tag in news_tags:
                ...
                if news_tag.root.tag == "div":
                    con_imgs = self.parse_many_img(response, news_tag)
                    if con_imgs:
                        content.extend(con_imgs)
        return content
