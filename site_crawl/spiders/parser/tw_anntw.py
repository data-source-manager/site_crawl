...
class TwAnntwParser(BaseParser):
    name = 'anntw'
    
    # 站点id
    site_id = "f2c43e67-d3cb-48da-96e8-e43ddb08d135"
    # 站点名
    site_name = "台湾醒报"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "f2c43e67-d3cb-48da-96e8-e43ddb08d135", "source_name": "台湾醒报", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8cd272-fe6f-11ec-a30b-d4619d029786", "国际", "https://anntw.com/categories/international?page=1", "政治"),
            ("4a8cd218-fe6f-11ec-a30b-d4619d029786", "政治", "https://anntw.com/categories/politics?page=1", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h4[@class="article-title"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//div[@class="span12 content-pad"]/h3/text()').extract_first(default="")
        return title.strip() if title else ""

    def get_author(self, response) -> list:
        author_list = []

        authors = response.xpath('//span[@class="text-toned bold author"]/a/text()').extract()
        if authors:
            for au in authors:
                if au.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="cover side fade-in"]/img|//div[@class="markdown-body"]/*')
        if news_tags:
    ...
    def get_read_count(self, response) -> int:
        read_count = 0
        read_count_str = response.xpath('//div[@class="meta-block"]/span[3]/text()').get()
        if read_count_str:
            read_count = int(re.findall('點閱 (.*) 次', read_count_str)[0])
        return read_count
