...
class TwNpforgParser(BaseParser):
    name = 'npforg'
    
    # 站点id
    site_id = "8a758cab-144c-4b14-9199-c4896bf64089"
    # 站点名
    site_name = "国民党国家政策研究基金会全球咨询网"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "8a758cab-144c-4b14-9199-c4896bf64089", "source_name": "国民党国家政策研究基金会全球咨询网", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8c84f2-fe6f-11ec-a30b-d4619d029786", "最新文章", "https://www.npf.org.tw/morenewarticle", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@id="listarticleright"]')
        if news_urls:
            for news_url in news_urls:
                rsp_url = news_url.xpath('./p/a/@href').get()
                rsp_time = news_url.xpath('./p/span/span/span/@title').get()
                url = urljoin(response.url, rsp_url)
                year, mt, day, tm = re.findall('(\d{4})年(\d{1,2})月(\d{1,2})日 (.*)', rsp_time)[0]
                times_ = year + '-' + mt + '-' + day + ' ' + tm
                self.Dict[url] = {'time': times_}
                yield url
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath(
            '//div[@id="articlepicposition"]/img|//div[@id="articlebody"]/*|//div[@id="articlebody"]')
        if news_tags:
