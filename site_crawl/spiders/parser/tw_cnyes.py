...
class TwCnyesParser(BaseParser):
    name = 'cnyes'
    
    # 站点id
    site_id = "04109d5f-d0ab-4705-8cf6-4ec905dbda1f"
    # 站点名
    site_name = "巨亨网新闻"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "04109d5f-d0ab-4705-8cf6-4ec905dbda1f", "source_name": "巨亨网新闻", "direction": "tw", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("1aae4b95-cda7-4eed-b9f2-c5f5dd26d74d", "头条", "https://news.cnyes.com/news/cat/headline?exp=a", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//a[@class="_1Zdp"]/@href').extract() or ""
        if news_urls:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//a[@class="_3Yas"]/span/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//article//figure/img|//div[@class="_2E8y"]/*')
        if news_tags:
