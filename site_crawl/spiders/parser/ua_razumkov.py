...
class UaRazumkovParser(BaseParser):
    name = 'razumkov'
    
    # 站点id
    site_id = "898c6498-8be8-4524-a3a1-26ce770e33c7"
    # 站点名
    site_name = "拉祖姆科夫中心"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "898c6498-8be8-4524-a3a1-26ce770e33c7", "source_name": "拉祖姆科夫中心", "direction": "ua", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8d040e-fe6f-11ec-a30b-d4619d029786", "文章和采访", "https://razumkov.org.ua/statti", "政治"),
            ("4a8d0396-fe6f-11ec-a30b-d4619d029786", "新闻稿", "https://razumkov.org.ua/en/sociology/press-releases", "政治"),
            ("4a8d04ae-fe6f-11ec-a30b-d4619d029786", "消息", "https://razumkov.org.ua/novyny", "政治"),
            ("4a8d0454-fe6f-11ec-a30b-d4619d029786", "评论", "https://razumkov.org.ua/komentari", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h2[@class="tagItemTitle"]/a/@href|'
                                   '//h3[@class="catItemTitle"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        if "statti" or "komentari" or "novyny" in response.url:
            times_ = response.xpath('//span[@class="itemDateCreated"]/text()[2]').extract_first()
            day, mt, year = re.findall('(\d{1,2}) (.*) (\d{4})', times_)[0]
            time_ = year + '-' + translatetTime.uaDict[mt] + '-' + day
            if time_:
                pub_time = datetime_helper.fuzzy_parse_timestamp(time_)
                return str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(pub_time)))
        else:
            time_ = response.xpath('//span[@class="itemDateCreated"]/text()[2]').extract_first()
            if time_:
                pub_time = datetime_helper.fuzzy_parse_timestamp(time_)
                return str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(pub_time)))
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="itemFullText"]/*|//div[@class="itemFullText"]/div/table')
        if news_tags:
            for news_tag in news_tags:
                if type(news_tag.root) == str:
                    ...
                    if news_tag.root.tag in ["h2", "h3", "h5", "span", "p", "em", "strong"]:
