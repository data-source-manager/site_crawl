...
class UsaClaCsoParser(BaseParser):
    name = 'clacso'
    
    # 站点id
    site_id = "a3ea0d4f-e39e-4f54-9ae9-e872a2e7e1c0"
    # 站点名
    site_name = "拉丁美洲社会科学理事会"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "a3ea0d4f-e39e-4f54-9ae9-e872a2e7e1c0", "source_name": "拉丁美洲社会科学理事会", "direction": "usa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f18de-fe6f-11ec-a30b-d4619d029786", "消息", "https://www.clacso.org/noticias-y-novedades/page/1/", "政治"),
        ]
    ]
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//div[@class="tags-links"]/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="entry-thumbnail"]/img|//div[@class="pf-content"]/*')
        if news_tags:
