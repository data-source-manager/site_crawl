...
class PeIepParser(BaseParser):
    name = 'iep'
    # 站点id
    site_id = "769f8290-9f4e-4bb4-9f59-7741fdb2879b"
    # 站点名
    site_name = "秘鲁研究院"
    # 板块信息
    channel = [
        {
            ...
            **{"site_id": "769f8290-9f4e-4bb4-9f59-7741fdb2879b", "source_name": "秘鲁研究院", "direction": "pe", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f5768-fe6f-11ec-a30b-d4619d029786", "消息", "https://iep.org.pe/noticias/page/1/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//div[@class="content"]/h3/a/@href').extract() or ""
        if news_urls:
    ...
    def get_pub_time(self, response) -> str:
        ...
        times_ = response.xpath('//time[@class="value-datetime"]/text()').extract_first()
        day, month, year = re.findall('(\d{1,2}) (.*) (\d{4})', times_)[0]
        time_ = year + "-" + translatetTime.mxDict[month] + "-" + day
        if time_:
    ...
    def get_tags(self, response) -> list:
        ...
        tags = response.xpath('//span[@class="cats"]/a/text()').extract()
        if tags:
            for tag in tags:
                if tag.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="post-content description "]/p/img|//div[@class="llamada"]/*|'
                                   '//div[@class="post-content description "]/p|//div[@class="llamada"]/p/a')
        if news_tags:
