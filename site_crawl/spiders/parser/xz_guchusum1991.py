...
class XzGuchusum1991Parser(BaseParser):
    name = 'guchusum1991'
    
    # 站点id
    site_id = "dd942ae7-8eba-4fcf-b3c0-3303fae515af"
    # 站点名
    site_name = "西藏“9-10-3”运动"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "dd942ae7-8eba-4fcf-b3c0-3303fae515af", "source_name": "西藏“9-10-3”运动", "direction": "xz", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8f501a-fe6f-11ec-a30b-d4619d029786", "新闻", "https://en.guchusum1991.org/category/news/page/1/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h2[@class="blog-entry-title entry-title"]/a/@href').extract() or ""
        if news_urls:
            for news_url in list(set(news_urls)):
                yield urljoin(response.url, news_url)

    def get_title(self, response) -> str:
        title = response.xpath('//h2[@itemprop="headline"]/text()').extract_first(default="")
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@itemprop="text"]/*|'
