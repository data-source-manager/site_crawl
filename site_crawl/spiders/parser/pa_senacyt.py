...
class PaSenaCytParser(BaseParser):
    name = 'pa_senacyt'
    
    # 站点id
    site_id = "72f562af-e3e7-4ba3-a4f9-985e8b5cc6de"
    # 站点名
    site_name = "国家科学、技术与创新部"
    # 板块信息
    channel = [
        {
            # 板块默认字段(站点id, 站点名, 站点地区)
            **{"site_id": "72f562af-e3e7-4ba3-a4f9-985e8b5cc6de", "source_name": "国家科学、技术与创新部", "direction": "pa", "if_front_position": False}, 
            # (板块id, 板块名, 板块URL, 板块类型)
            **{"board_id": board_id, "site_board_name": board_name, "url": board_url, "board_theme": board_theme}
        }
        for board_id, board_name, board_url, board_theme in [
            ("4a8ef82c-fe6f-11ec-a30b-d4619d029786", "消息", "https://www.senacyt.gob.pa/category/noticias/page/1/", "政治"),
        ]
    ]
    ...
    def parse_list(self, response) -> list:
        news_urls = response.xpath('//h2[@class="title_home"]/a/@href').extract() or ""
        if news_urls:
    ...
    def get_author(self, response) -> list:
        ...
        authors = response.xpath('//span[@class="byline"]/a/text()').extract()
        if authors:
            for au in authors:
                if au.strip():
    ...
    def get_content_media(self, response) -> list:
        ...
        news_tags = response.xpath('//div[@class="post_image"]/img|//div[@class="post_content"]/*')
        if news_tags:
    ...
    def get_read_count(self, response) -> int:
        read_count = 0
        read_count_str = response.xpath('//span[@class="post_views"]/text()').get()
        if read_count_str:
            read_count = int(re.findall('(.*) vistas ', read_count_str)[0])
        return read_count
